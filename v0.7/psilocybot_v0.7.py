...
async def dose(ctx, query: str):
	...
	y = api.json()
	dicts = {
		"Cannabinoid": 0x00ff00,
		"Lysergamides": 0x9900ff,
		"Entheogens": 0x08d489,
		"Dissociative": 0x230585,
		"Depressants": 0x162e52,
		"Stimulants": 0xff0000,
		"Entactogens": 0xffff00,
		"Nootropics": 0x00ff8c,
		"Antidepressants": 0x002a6e,
		"Antipsychotics": 0x0000ff,
		"Miscellaneous": 0xe62cdc
	}
	colorful = 0x00ff00
	for subs in y["data"]["substances"]:
		chemicals = subs["class"]
		z = chemicals["chemical"]
		doses = subs["roas"][0]["dose"]
		light = doses["light"]
		common = doses["common"]
		strong = doses["strong"]
	...
	if zstr in dicts:
		...
		colorful = 0x00ff00
	embed=discord.Embed(title=f"{subs['name']}", description=summ, url=f"{subs['url']}", color=colorful)
	embed.add_field(name="Class", value=zstr, inline=False)
	embed.add_field(name="-", value="-----------------", inline=False)
